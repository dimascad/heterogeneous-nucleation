...
def _(theta_slider, mo):
    ...
    def draw_geometry_panel(draw, theta_deg, px, py, pw, ph, fonts):
        ...
        # Labels using Y for gamma (safe ASCII)
        ...
        sf = S(theta_deg)
        draw.text((px + 12, py + 65), f"theta = {theta_deg:.0f} deg", fill=GREEN, font=f_large)
        draw.text((px + 12, py + 88), f"S(theta) = {sf:.4f}", fill=CYAN, font=f_normal)
        if theta_deg < 30:
        ...
        draw.text((px + pw - 8, ly + 26), "Ysl = Ysn + Ynl*cos(theta)", fill=WHITE, anchor="rt", font=f_normal)

    def draw_shape_factor_panel(draw, theta_deg, px, py, pw, ph, fonts):
        f_small, f_normal, f_medium, f_large, f_title, f_bigtitle = fonts
        draw.rectangle([px, py, px+pw, py+ph], fill=PANEL_BG, outline=LIGHT_GRAY)
        draw.text((px + pw//2, py + 18), "SHAPE FACTOR S(theta)", fill=YELLOW, anchor="mm", font=f_bigtitle)
        sf = S(theta_deg)
        draw.text((px + pw//2, py + 42), f"S({theta_deg:.0f} deg) = {sf:.4f}", fill=CYAN, anchor="mm", font=f_medium)
        ...
        draw.text((plot_x + plot_w//2, plot_y + plot_h + 32), "Contact Angle theta (degrees)", 
        ...
        draw.text((px + 6, plot_y - 22), "S(theta)", fill=CYAN, font=f_large)
        ...
        draw.text((eq_x, eq_y), "S = (2+cos)(1-cos)^2/4", fill=CYAN, anchor="mm", font=f_normal)
    ...
    def draw_barrier_panel(draw, theta_deg, px, py, pw, ph, fonts):
        ...
        draw.text((eq_x, eq_y), "dG*(het) = S*dG*(hom)", fill=ORANGE, anchor="mm", font=f_medium)
...
def _(mo):
    ...
    This interactive visualization demonstrates how **contact angle** affects heterogeneous nucleation:

    - **Left panel**: Shows the nucleus geometry as a spherical cap on a substrate, with surface tension vectors and the contact angle
    - **Middle panel**: The shape factor S(theta) curve - this determines how much the nucleation barrier is reduced
    ...
    **S(theta) = (2 + cos(theta))(1 - cos(theta))^2 / 4**

    This determines the heterogeneous nucleation barrier:
    
    **dG*_het = S(theta) * dG*_hom**

    - At theta -> 0 (perfect wetting): S -> 0, barrier nearly eliminated
    - At theta = 90: S = 0.5, barrier halved  
    - At theta -> 180 (no wetting): S -> 1, no reduction (same as homogeneous)
